...
def gerenciar_clientes(request):
    ...
    return render(request, 'gerenciar_clientes.htm', {'clientes': clientes, 'nome_completo': nome_completo, 'email': email})

@staff_member_required 
def cliente(request, cliente_id):
    cliente = User.objects.get(id=cliente_id)
    exames = SolicitacaoExame.objects.filter(usuario=cliente)
    return render(request, 'cliente.htm', {'cliente': cliente, 'exames': exames})

@staff_member_required 
def exame_cliente(request, exame_id):
    exame = SolicitacaoExame.objects.get(id=exame_id)
    return render(request, 'exame_cliente.htm', {'exame': exame})
